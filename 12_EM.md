...
$$q^{\ast}(z) = \arg\min_{q}  D_{\text{KL}}(q(z)\|p(z|x,\theta^{(t)})) = p(z|x,\theta^{(t)})$$

Setting $q(z) = p(z|x,\theta^{(t)})$ makes $D_{\text{KL}} = 0$, so:
$$\text{ELBO}(q^{\ast}, \theta^{(t)}) = \log p(x|\theta^{(t)})$$
...
$$\text{ELBO}(q^{\ast},\theta) = \int p(z|x,\theta^{(t)})\log\frac{p(x,z|\theta)}{p(z|x,\theta^{(t)})}\,dz$$
...
$$\theta^{(t+1)} = \arg\max_\theta \text{ELBO}(q^{\ast},\theta) = \arg\max_\theta Q(\theta,\theta^{(t)})$$
