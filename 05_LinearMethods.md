...
If $x$ is primal feasible (constraints satisfied), the inner max equals $f(x)$. If infeasible, the inner max is $+\infty$. So minimising $\Theta_\mathcal{P}$ is equivalent to the original problem. Optimal value: $p^{\ast}$.

**Dual problem:**

$$\max_{\alpha\geq 0,\beta} \Theta_\mathcal{D}(\alpha,\beta) = \max_{\alpha\geq 0,\beta}\left[\min_x \mathcal{L}(x,\alpha,\beta)\right]$$

$\Theta_\mathcal{D}$ is always concave (even if the primal is non-convex). Optimal value: $d^{\ast}$.

### 2.4 Weak and Strong Duality

**Weak duality:** Always $d^{\ast} \leq p^{\ast}$ (the dual is a lower bound on the primal).

**Strong duality:** Under certain conditions (e.g., **Slater's condition**), $d^{\ast} = p^{\ast}$.
...
If $x^{\ast}$, $\alpha^{\ast}$, $\beta^{\ast}$ satisfy all four KKT conditions simultaneously, then $x^{\ast}$ is primal-optimal and $(\alpha^{\ast},\beta^{\ast})$ are dual-optimal (under strong duality):

1. **Primal feasibility:** $g_i(x^{\ast})\leq 0$ and $h_i(x^{\ast})=0$.
2. **Dual feasibility:** $\alpha_{i}^{\ast}\geq 0$.
3. **Complementary slackness:** $\alpha_{i}^{\ast}g_i(x^{\ast})=0$ for all $i$.  
   (Either the constraint is active $g_i=0$, or the multiplier is zero $\alpha_i=0$.)
4. **Lagrangian stationarity:**

$$\nabla_x\mathcal{L}(x^{\ast},\alpha^{\ast},\beta^{\ast}) = \nabla f(x^{\ast}) + \sum_i\alpha_{i}^{\ast}\nabla g_i(x^{\ast}) + \sum_i\beta_{i}^{\ast}\nabla h_i(x^{\ast}) = 0$$
...
$$\lambda^{\ast} = \sum_{i:\alpha_i>0}\alpha_{i}^{\ast}y_ix_i, \qquad \lambda_0^{\ast} = 1 - \min_{i:y_i=1}(\lambda^{\ast})^\top x_i$$

**Support vectors:** points with $\alpha_{i}^{\ast}>0$ — they lie exactly on the margin boundary and are the only points that determine $\lambda^{\ast}$.
